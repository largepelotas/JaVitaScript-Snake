...
Author of record: Quinn. Target device: real hacked PS Vita (Ensō/HENkaku/h-encore).
...
0.4  Stop and ask Quinn when you hit any of these:
...
2.1  Environment. All build work happens inside WSL2 Ubuntu, not Windows
     ...
     Confirm WSL2 Ubuntu is present; if not, tell Quinn to run
...
2.3  VitaSDK install. Prefix is ~/vitasdk, NOT /usr/local/vitasdk (Quinn's
...
2.4  Confirm SDL2_ttf is present for the Vita target. If it is NOT, do not
     ...
     tell Quinn you switched. Check with:
...
2.6  Vita3K. Tell Quinn to install the Windows build of Vita3K and complete
     its firmware setup. You will not drive it; you will hand Quinn a .vpk and
...
4.3  State machine states: WELCOME, READY, PLAYING, PAUSED, DEAD, WON.
     ...
     AMENDED 2026-07-31 (Quinn): READY was added. The original has a distinct
...
6.5  Input mapping. This is the highest-risk item in the project because you
     ...
     - Ship a hidden diagnostic: hold L+R at the welcome screen to display,
       ...
       same to log.txt. Quinn runs this once on hardware and reports the real
...
7.2  Vita packaging macros: vita_create_self then vita_create_vpk.
     - VITA_APP_NAME  "Snake"
     - VITA_TITLEID   "SNEK00001"  (Quinn's choice, 2026-07-31; supersedes the
...
7.4  Fast iteration on hardware. Have Quinn install VitaShell and
     ...
     docs/DEPLOY.md so Quinn can run it as a one-liner.

7.5  Crash handling. If it crashes on hardware, Quinn will have a core dump in
...
==============================================================================
8. PHASE 6 - VALIDATION (QUINN RUNS THIS, YOU WRITE IT)
==============================================================================

Produce docs/TESTPLAN.md containing a numbered checklist Quinn can execute in
one sitting on hardware. Include at minimum:
  ...
  4.  L+R diagnostic reports button indices; Quinn records them
...
Ask Quinn to send back log.txt and the button indices from item 4.
...
vita-snake/
  ...
  docs/
    ...
    TESTPLAN.md                hardware checklist for Quinn
  src/
...
==============================================================================
  ...
  Phase 6  TESTPLAN.md, hand to Quinn, fix what hardware reveals
...
==============================================================================
 ...
 11.1  RESOLVED 2026-08-01. Vita SDL2 controller button indices are confirmed on
       ...
       Original item: indices are unverified until Quinn runs the L+R
       ...
       Quinn confirms once on device; a disagreement is a one-line fix to
       ...
       Quinn re-ran item 4 on the fixed build the same day: cross reports 2.
